...
function calculate_cutscene_array()
    ...
    if ReadByte(journal_byte_pointer+journal_byte_value_offset_axel, true) > 0 then
        fire_unlocked = true
    end
    if ReadByte(journal_byte_pointer+journal_byte_value_offset_larxene, true) > 0 then
        thunder_unlocked = true
    end
    if ReadByte(journal_byte_pointer+journal_byte_value_offset_riku, true) > 0 then
        aero_unlocked = true
    end
    if ReadByte(journal_byte_pointer+journal_byte_value_offset_riku_3, true) > 0 then
        mega_potion_unlocked = true
    end
    if ReadByte(journal_byte_pointer+journal_byte_value_offset_larxene_2, true) > 0 then
